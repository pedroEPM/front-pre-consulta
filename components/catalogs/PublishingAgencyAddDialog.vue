  ...
  <v-dialog max-width="400" v-model="dialog">
    <v-card>
      <v-toolbar elevation="0">
        <v-card-title>Agregar agencia de publicación</v-card-title>
 ...
 data(){
  ...
  methods : {
    ...
    async addPublishingAgency(){
      ...
      try {
        const response = await this.$axios.$post('/addAgency', formData, {
        ...
        if(response){
          ...
          this.$store.commit(
            'openSnack',
            'La agencia de publicación ha sido registrada exitosamente'
          );
        }
        this.cleanAndClose();
        this.loading = false;
        ...
        this.$store.commit(
          'openSnack',
          'Ha ocurrido un error, por favor vuelva a intertarlo'
        );
      }
    },

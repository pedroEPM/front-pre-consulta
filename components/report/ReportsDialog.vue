    ...
    <v-card-text v-if="!loading">
      ...
      >
        ...
        <template v-slot:[`item.state`]="{ item }">
          <v-chip @click="$refs.reportEditDialog.openDialog( item )" small :color="item.state == 'Finalizado' ? 'success' : item.state == 'Proceso' ? 'warning' : 'error'  ">{{ item.state }}</v-chip>
        </template>
        <template v-slot:[`item.action`]="{ item }">
          <v-icon class="mr-2" @click="$refs.reportEditDialog.openDialog( item )"> mdi-pencil </v-icon>
          <v-icon class="mr-2" @click="$refs.reportDeleteDialog.openDialog( item )"> mdi-delete </v-icon>
        </template>
      </v-data-table>
    </v-card-text>

    <LoaderCard v-show="loading" />
    <ReportAddDialog ref="reportAddDialog" />
    <ReportEditDialog ref="reportEditDialog" />
    <ReportDeleteDialog ref="reportDeleteDialog" />
  </v-card>
</template>

<script>
  import ReportEditDialog from '~/components/report/ReportEditDialog';
  import ReportDeleteDialog from '~/components/report/ReportDeleteDialog';
  import ReportAddDialog from '~/components/report/ReportsAddDialog';
    ...
    data(){
      return {
        ...
        reportsHeaders : [
          ...
          { text : 'Estado', sortable: true, align: "center", value : 'state'},

<template>
  <v-card>
    <v-toolbar flat>
      ...
      <v-spacer />
      <v-btn color="primary" @click="$refs.bannersAddDialog.openDialog()">Agregar imagen</v-btn>
    </v-toolbar>
    <v-card-text>
      <v-layout row wrap>
        <v-flex xs12 md4 v-for="( item, index ) in banner2" :key="index">
          <v-card color="transparent" outlined>
            <img class="news-image" @click="$refs.bannersEditDialog.openDialog( item._id )" height="250" :src="imgPath + item.imgUrl" />
          </v-card>
    ...
    <BannersAddDialog ref="bannersAddDialog" />
    <BannersEditDialog ref="bannersEditDialog" />
  </v-card>
</template>

<script>
  import BannersAddDialog from '~/components/principalPage/BannersAddDialog';
  import BannersEditDialog from '~/components/principalPage/BannersEditDialog';
  export default {
    components : {
      BannersAddDialog,
      BannersEditDialog

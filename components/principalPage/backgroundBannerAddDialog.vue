    ...
    <v-card>
      <v-card-title class="pa-0">
        <v-toolbar dark color="primary">
          <v-spacer />
          <v-toolbar-title>Creación de fondo </v-toolbar-title>
          <v-spacer />
    ...
    <v-dialog v-model="confirm" max-width="300">
      <v-card :loading="loading">
        ...
        <v-card-text v-if="!loading">
          ¿Esta seguro de proceder con la carga del fondo?
        </v-card-text>
        <v-card-text v-else>
          Cargando el fondo
        </v-card-text>
        <v-card-actions v-if="!loading">
          ...
          <v-btn color="primary" @click="addBackgroundBanner()">Aceptar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
<script>
  ...
  export default {
    ...
    methods : {
      ...
      async addBackgroundBanner(){
        this.loading = true;
        let formData = new FormData();
        let body = {

          idUser : localStorage.user_id,
          active : this.state,
        ...
        try {
          const response = await this.$axios.$post('/addBackgroundBanner', formData, {
          ...
          this.$store.commit(
            'openSnack',
            'El fondo ha sido agregado exitosamente'
          );

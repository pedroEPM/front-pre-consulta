  ...
  <v-dialog max-width="300" v-model="dialog">
    <v-card :loading="loading">
      ...
      <v-card-text v-if="!loading">
        ¿Esta seguro de querer eliminar el fondo?
      </v-card-text>
      <v-card-text v-if="loading">Eliminando fondo</v-card-text>
      <v-card-actions v-if="!loading">
        <v-spacer />
        <v-btn text color="" @click="dialog = false">Cancelar</v-btn>
        <v-btn text color="red" @click="deleteBackgroundBanner()">Eliminar</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
<script>
  export default {
    ...
    methods : {
      ...
      async deleteBackgroundBanner(){
        try {
          this.loading = true;
          const response = await this.$axios.$delete('/backgroundbanner/' + this.id, {
      ...
      async cleanAndClose(){
        ...
        await this.$parent.$parent.$parent.$parent.$parent.getBackgroundBanners();
